...
js/
  ...
  data/
    ...
    photos.js           # photo manifest — GENERATED by npm run photos
...
assets/
  ...
  photos/               # gallery WebPs (+ thumbs/), built by the photo pipeline
  resume/               # résumé PDFs (AI/ML and Data Analytics variants)
data/
  maps-src/             # Natural Earth source data for the map build
photos-original/        # source photos (git-ignored; see its README)
scripts/
...
committed — see `photos-original/README.md`). The pipeline converts them to
committed web assets:
...
first generation. You can also toggle `featured` by hand in
`js/data/photos.js`.
